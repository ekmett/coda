...
data Opening a = Opening {-# unpack #-} !(LocatedPair a) !(Cat a)
data Closing a = Closing !(Cat a) {-# unpack #-} !(LocatedPair a)
...
deriving instance (Relative a, Read (LayoutMode a), Read (Pair a), Read a) => Read (Dyck a)
...
-- | O(1)
token :: (Default (LayoutMode a), Relative a) => Dyck a -> a -> Dyck a
token (Dyck l ms r s _ e) a = Dyck l ms r (snocCat s a) def e

layoutToken :: Relative a => Dyck a -> LayoutMode a -> a -> Dyck a
layoutToken (Dyck l ms r s _ e) i a = Dyck l ms r (snocCat s a) i e

-- | O(1)
close :: (Default (LayoutMode a), Eq (Pair a), AsRich a a, Relative a) => Dyck a -> LocatedPair a -> Dyck a
close (Dyck l ms (r' :> Opening dp rs) s _ e) dq
 | extract dp == extract dq  = Dyck l ms r' (Cat.singleton $ Rich $ Nested dp (rs<>s)) def e
 | !f <- MismatchError dp dq = Dyck l ms r' (Cat.singleton $ Rich $ Mismatch f (rs<>s)) def (snocCat e f)
close (Dyck l ms _ s _ e) dq = Dyck (snocCat l $ Closing (ms <> s) dq) mempty mempty mempty def e

-- | O(1)
open :: (Default (LayoutMode a), Relative a) => Dyck a -> LocatedPair a -> Dyck a
...
instance Default (LayoutMode a) => Default (Dyck a) where
...
instance (Default (LayoutMode a), Eq (Pair a), AsRich a a, Relative a) => Monoid (Dyck a) where
  mempty = Dyck mempty mempty mempty mempty def mempty
  mappend = (<>)
  {-# inline mappend #-}

instance (Default (LayoutMode a), Eq (Pair a), AsRich a a, Relative a) => RelativeMonoid (Dyck a)
