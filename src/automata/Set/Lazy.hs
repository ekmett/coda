...
import Prelude hiding (filter,foldl,foldr,null,map,take,drop,splitAt,sum,product)
...
link x l@(Bin sizeL y ly ry) r@(Bin sizeR z lz rz)
  | delta*sizeL < sizeR  = balanceLn (1 + sizeL + sizeR) z (link x l lz) rz
  | delta*sizeR < sizeL  = balanceRn (1 + sizeL + sizeR) y ly (link x ry r)
  | otherwise            = Bin (1 + sizeL + sizeR) x l r

-- insertMin and insertMax don't perform potentially expensive comparisons.
insertMax,insertMin :: a -> Set a -> Set a
insertMax x t = case t of
  Tip -> singleton x
  Bin s y l r -> balanceRn (s+1) y l (insertMax x r)

insertMin x t = case t of
  Tip -> singleton x
  Bin s y l r -> balanceLn (s+1) y (insertMin x l) r
...
merge l@(Bin sizeL x lx rx) r@(Bin sizeR y ly ry)
  | delta*sizeL < sizeR = balanceLn (sizeL + sizeR) y (merge l ly) ry
  | delta*sizeR < sizeL = balanceRn (sizeL + sizeR) x lx (merge rx r)
  | otherwise           = glue l r
...
glue l@(Bin sl xl ll lr) r@(Bin sr xr rl rr)
  | sl > sr = let !(m :*: l') = maxViewSure xl ll lr in balanceRn (sl + sr) m l' r
  | otherwise = let !(m :*: r') = minViewSure xr rl rr in balanceLn (sl + sr) m l r'
...
balanceLn :: Size -> a -> Set a -> Set a -> Set a
balanceLn !s x l r = case r of
  Tip -> case l of
    Tip -> Bin s x Tip Tip
    Bin _ _ Tip Tip -> Bin s x l Tip
    Bin _ lx Tip (Bin _ lrx _ _) -> Bin s lrx (Bin 1 lx Tip Tip) (Bin 1 x Tip Tip)
    Bin _ lx ll@(Bin _ _ _ _) Tip -> Bin s lx ll (Bin 1 x Tip Tip)
    Bin _ lx ll@(Bin lls _ _ _) lr@(Bin lrs lrx lrl lrr)
      | lrs < ratio*lls -> Bin s lx ll (Bin (1+lrs) x lr Tip)
      | otherwise -> Bin s lrx (Bin (1+lls+size lrl) lx ll lrl) (Bin (1+size lrr) x lrr Tip)
    ...
    Bin ls lx ll lr
      | ls > delta*rs  -> case (ll, lr) of
           (Bin lls _ _ _, Bin lrs lrx lrl lrr)
             | lrs < ratio*lls -> Bin s lx ll (Bin (1+rs+lrs) x lr r)
             | otherwise -> Bin s lrx (Bin (1+lls+size lrl) lx ll lrl) (Bin (1+rs+size lrr) x lrr r)
           (_, _) -> error "Failure in Data.Map.balanceL"
      | otherwise -> Bin s x l r
{-# NOINLINE balanceLn #-}
...
balanceRn :: Size -> a -> Set a -> Set a -> Set a
balanceRn !s x l r = case l of
  Tip -> case r of
    Tip -> Bin s x Tip Tip
    Bin _ _ Tip Tip -> Bin s x Tip r
    Bin _ rx Tip rr@(Bin _ _ _ _) -> Bin s rx (Bin 1 x Tip Tip) rr
    Bin _ rx (Bin _ rlx _ _) Tip -> Bin s rlx (Bin 1 x Tip Tip) (Bin 1 rx Tip Tip)
    Bin _ rx rl@(Bin rls rlx rll rlr) rr@(Bin rrs _ _ _)
      | rls < ratio*rrs -> Bin s rx (Bin (1+rls) x Tip rl) rr
      | otherwise -> Bin s rlx (Bin (1+size rll) x Tip rll) (Bin (1+rrs+size rlr) rx rlr rr)
  Bin ls _ _ _ -> case r of
    Tip -> Bin s x l Tip
    Bin rs rx rl rr
      | rs > delta*ls  -> case (rl, rr) of
           (Bin rls rlx rll rlr, Bin rrs _ _ _)
             | rls < ratio*rrs -> Bin s rx (Bin (1+ls+rls) x l rl) rr
             | otherwise -> Bin s rlx (Bin (1+ls+size rll) x l rll) (Bin (1+rrs+size rlr) rx rlr rr)
...
{--------------------------------------------------------------------
  Extensions
--------------------------------------------------------------------}
newtype Merge a = Merge { getMerge :: Set a }

instance Semigroup (Merge a) where
  Merge m <> Merge n = Merge (merge m n)

instance Monoid (Merge a) where
  mempty = Merge empty
  mappend (Merge m) (Merge n) = Merge (merge m n)

product :: Set a -> Set b -> Set (a, b)
product as bs = getMerge $ foldMap (\a -> Merge $ mapMonotonic (a,) bs) as

sum :: Set a -> Set b -> Set (Either a b)
sum as bs = merge (mapMonotonic Left as) (mapMonotonic Right bs)
