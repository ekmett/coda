...
def start(i: ℕ) : cursor 0 i := begin
...
def step: Π(n i : ℕ) (c : cursor n i), action n i c := begin
  introv,
  ...
  begin -- (k-1) % 2 = 1, go right
    ...
    exact r_equals_1, apply (@cursor.mk (n+2*s) i (s*2) (km1/2) _); refine (and.intro _ ds),
    begin -- proof
